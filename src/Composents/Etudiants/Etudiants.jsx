...
function Etudiants() {
  ...
  const numberOfPage = Math.ceil(datas?.length / postsParPages)
  const postsCourrent = datas.slice(premierePage, dernierPost);
  console.log(postsCourrent);

  return (
    <Col className='col-6 offset-2 g-3 ps-5 mt-5'>
      <Row className="mx-auto border rounded my-2 shadow">
        <Col className="col-12 bg-light p-3 text-start fw-bold">STUDENT GRADING SYSTEM</Col>
        <Col className="col-12 p-3">
          <Row>
            <Col className="col-6 d-flex align-items-center">
                <Form.Select aria-label="Default select example" className="w-25" onChange={(e) => setPostsParPages(e.target.value)}>
                ...
                <div className="mx-3">records par page</div>
            </Col>
            <Col className="col-6">
              ...
              {postsCourrent.map(post => (
                <tr key={post.id}>
                  ...
                  <Row className="align-items-center">
                    <Col className="col-6">
                      <div className="text-start"> {pageCourant} / {numberOfPage} de  {datas.length} Etudinats</div>
                    </Col>
                    <Col className="col-6">
                      ...
                      {
                        pageCourant === 1 ? null : (
                        <button type="button" class="btn btn-outline-secondary" onClick={() => setPageCourant(v => v - 1)}>
                          <WiDirectionLeft className="fs-4"/>
                          Previous
                        </button>
                        )
                      }
                        ...
                        <button type="button" class="btn btn-outline-secondary" onClick={() => setPageCourant(v => v + 1)}>
                          Next
                          <WiDirectionRight  className="fs-2"/>
                        </button>
